  ...
  class RuleSet
    ...
    def get_value(property)
      ...
      properties = @declarations.inject('') do |val, (key, data)|
        ...
        val << "#{data[:value]}#{importance}; " if key.strip == property
        val
      end
    ...
    def each_declaration # :yields: property, value, is_important
      ...
      decs.each do |property, data|
        value = data[:value]
        yield property.strip, value.strip, data[:is_important]
      end
    end
      ...
      {'margin'       => 'margin-%s',
       ...
       'border-width' => 'border-%s-width'}.each do |property, expanded|
        ...
        if foldable.length == 4
          ...
          directions.each { |d| values[d.to_sym] = @declarations[expanded % d][:value].strip }
